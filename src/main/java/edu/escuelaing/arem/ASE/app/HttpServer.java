...
public class HttpServer {
    ...
    private static File resolveStaticFile(String requestPath) throws IOException {
        ...
        // base donde se buscarán los recursos (maven copy target/classes + staticFilesDirectory)
        File base = new File("target/classes" + staticFilesDirectory);
        File requested = new File(base, decoded);
        ...
        if (!requestedCanonical.startsWith(baseCanonical)) {
            // intento de acceso fuera del directorio base -> rechazar
            return null;
        }
